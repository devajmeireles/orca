...
export function ExperimentalPane({
  ...
  const showAgentTerminalPopover = matchesSettingsSearch(searchQuery, [
    EXPERIMENTAL_SEARCH_ENTRY.agentTerminalPopover
  ])
  ...
  return (
    <div className="space-y-4">
      ...
      {showAgentTerminalPopover ? (
        <SearchableSetting
          title="Agent terminal popover"
          description="Hover an agent status in a worktree card to open its terminal without switching workspaces."
          keywords={EXPERIMENTAL_SEARCH_ENTRY.agentTerminalPopover.keywords}
          className="space-y-3 py-2"
        >
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 shrink space-y-0.5">
              <Label>Agent terminal popover</Label>
              <p className="text-xs text-muted-foreground">
                Hover an agent status in a worktree card to open its terminal without switching
                workspaces.
              </p>
            </div>
            <button
              type="button"
              role="switch"
              aria-checked={settings.experimentalAgentTerminalPopover}
              onClick={() =>
                updateSettings({
                  experimentalAgentTerminalPopover: !settings.experimentalAgentTerminalPopover
                })
              }
              className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border border-transparent transition-colors ${
                settings.experimentalAgentTerminalPopover
                  ? 'bg-foreground'
                  : 'bg-muted-foreground/30'
              }`}
            >
              <span
                className={`inline-block h-3.5 w-3.5 transform rounded-full bg-background shadow-sm transition-transform ${
                  settings.experimentalAgentTerminalPopover ? 'translate-x-4' : 'translate-x-0.5'
                }`}
              />
            </button>
          </div>
